"use client";

type GoogleAuthButtonProps = {
  mode: "login" | "register";
  /** Required for register: project code from the form. */
  getRegisterParams?: () => { code: string; phone?: string } | { error: string };
  /** Optional: show form error instead of alert. */
  onError?: (message: string) => void;
  disabled?: boolean;
  className?: string;
};

function GoogleMark() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
      <path
        fill="#FFC107"
        d="M43.6 20.5H42V20H24v8h11.3C33.7 32.7 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3 0 5.8 1.1 7.9 3l5.7-5.7C34.2 6.1 29.4 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.2-.1-2.3-.4-3.5z"
      />
      <path
        fill="#FF3D00"
        d="M6.3 14.7l6.6 4.8C14.7 16 19 13 24 13c3 0 5.8 1.1 7.9 3l5.7-5.7C34.2 6.1 29.4 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.3 35.3 26.8 36 24 36c-5.3 0-9.7-3.3-11.3-7.9l-6.5 5C9.6 39.6 16.2 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.2 4.1-4.1 5.6L37.4 39.3C40.5 36.9 44 31.8 44 24c0-1.2-.1-2.3-.4-3.5z"
      />
    </svg>
  );
}

export default function GoogleAuthButton({
  mode,
  getRegisterParams,
  onError,
  disabled,
  className = "",
}: GoogleAuthButtonProps) {
  return (
    <button
      type="button"
      className={`plat-btn plat-btn-google ${className}`.trim()}
      disabled={disabled}
      onClick={() => {
        if (mode === "register") {
          const result = getRegisterParams?.();
          if (!result) return;
          if ("error" in result) {
            if (onError) onError(result.error);
            else window.alert(result.error);
            return;
          }
          const params = new URLSearchParams({
            mode: "register",
            code: result.code,
          });
          if (result.phone) params.set("phone", result.phone);
          window.location.href = `/api/plataforma/auth/google?${params.toString()}`;
          return;
        }
        window.location.href = "/api/plataforma/auth/google?mode=login";
      }}
    >
      <GoogleMark />
      Continuar con Google
    </button>
  );
}
